...
var sectortime = props.globals.getNode("/Honda-CBR/this-sector-time");
var laptime = props.globals.getNode("/Honda-CBR/this-lap-time");
var racetime = props.globals.getNode("/Honda-CBR/this-race-time");
var inrange = 0;
...
var total_result = func{
	var maxlaps = getprop("/Honda-CBR/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/Honda-CBR/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/Honda-CBR/show-race-times") or 0;
	var show_l_or_f = getprop("/Honda-CBR/switch-last-fastest") or 0;
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/Honda-CBR/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/Honda-CBR/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/Honda-CBR/"~pa~"/sector["~n~"]/last-time") or 0;
		var ft  = getprop("/Honda-CBR/"~pa~"/sector["~n~"]/fastest-time") or 0;
		...
		if(thsec == n and racelap.getValue() > 0){
			...
			if(ct > 0){
				...
				setprop("Honda-CBR/this-sector-time-sec", result[0]);
				setprop("Honda-CBR/this-sector-time-min", result[1]);
			}
			if(ft > 0){
				var result = calc_time(ft);
				setprop("Honda-CBR/fastest-sector-time-sec", result[0]);
				setprop("Honda-CBR/fastest-sector-time-min", result[1]);
			}
			# only for mp
	...
	var fl = getprop("/Honda-CBR/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/Honda-CBR/last-lap-time") or 0;
	...
	if(fl > 0){
		...
		setprop("Honda-CBR/fastest-lap-time-sec", result[0]);
		setprop("Honda-CBR/fastest-lap-time-min", result[1]);
		setprop("Honda-CBR/fastest-lap-time-hour", result[2]);
	}
	if(lapt > 0){
		var result = calc_time(lapt);
		setprop("Honda-CBR/this-lap-time-sec", result[0]);
		setprop("Honda-CBR/this-lap-time-min", result[1]);
		setprop("Honda-CBR/this-lap-time-hour", result[2]);
	}
	# only for mp
...
var find_marker = func{
	...
	if(getprop("/Honda-CBR/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/Honda-CBR/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/Honda-CBR/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/Honda-CBR/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/Honda-CBR/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/Honda-CBR/"~pa~"/sector["~ln~"]/start-time") or 0;
		var lastsectorendtime = getprop("/Honda-CBR/"~pa~"/sector["~thissector.getValue()~"]/start-time") or 0;
		...
		setprop("/Honda-CBR/"~pa~"/sector["~ln~"]/last-time", lasttime);
		if(lasttime > 0 and lasttime < fastesttime or fastesttime == 0) setprop("/Honda-CBR/"~pa~"/sector["~ln~"]/fastest-time", lasttime);
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/Honda-CBR/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
		  setprop("/Honda-CBR/last-lap-time", totallapresult);
		  var fastestlap = getprop("/Honda-CBR/"~pa~"/fastest-lap") or 0;
		  if(totallapresult > 0 and totallapresult < fastestlap or fastestlap == 0) setprop("/Honda-CBR/"~pa~"/fastest-lap", totallapresult);
		  setprop("/Honda-CBR/"~pa~"/lap["~racelap.getValue()~"]/actual-time", totallapresult);
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/Honda-CBR/"~pa);
		
		setprop("/Honda-CBR/last-lap-time",0);
